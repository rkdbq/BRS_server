...
class UGATIT(object) :
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if step % self.print_freq == 0:
                ...
                cv2.imwrite(os.path.join('model', self.result_dir, self.dataset, 'img', 'A2B_%07d.png' % step), A2B * 255.0)
                cv2.imwrite(os.path.join('model', self.result_dir, self.dataset, 'img', 'B2A_%07d.png' % step), B2A * 255.0)
                self.genA2B.train(), self.genB2A.train(), self.disGA.train(), self.disGB.train(), self.disLA.train(), self.disLB.train()

            if step % self.save_freq == 0:
                self.save(os.path.join('model', self.result_dir, self.dataset, 'model'), step)
            ...
            if step % 1000 == 0:
                ...
                torch.save(params, os.path.join('model', self.result_dir, self.dataset + '_params_latest.pt'))
    ...
    def test(self):
        ...
        if not len(model_list) == 0:
            ...
            self.load(os.path.join('model', self.result_dir, self.dataset, 'model'), iter)
            print(" [*] Load SUCCESS")
        else:
            print(" [*] Load FAILURE")
            return
